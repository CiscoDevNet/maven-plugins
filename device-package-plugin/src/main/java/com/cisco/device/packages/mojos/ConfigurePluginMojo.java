...
public class ConfigurePluginMojo extends AbstractMojo {
    ...
    private static final String PLUGIN_ARTIFACT_ID = "device-package-plugin"; // $NON-NLS-1$
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        if (reactorProjects.size() > 1) {
            getLog().info("Configuring reactor projects to use the device-package-plugin...");
        } else {
            getLog().info("Configuring project to use the device-package-plugin...");
        }
